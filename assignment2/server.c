...
int main(int argc, char const *argv[])
{
    ...
    if (argc==1){
        ...
        if (pid == 0)
        {
            ...
            sprintf(sock_fd_str, "%d", server_fd);
            sprintf(argv_ori, "%s", argv[0]);

            char *args[] = {argv_ori, sock_fd_str, NULL};
            // re-exec passing in socket file descriptor
            execv(argv[0], args);
        }
        else
        {
